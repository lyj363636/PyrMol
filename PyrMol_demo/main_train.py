...
def train_sesstion(config,model_name,contrastive_weight = False,seeds=[0,100,200,300,400]):
    ...
    if model_name == "Version3_MultiSub_Contrastive":
        save_path = f"./{model_name}_{config['depth']}_sim{config['similarity_sub']}_tem{config['temperature']}_ctr{config['contrastive_weight']}/{config['data_name']}"  # for Version3_MultiSub_Multiview
...
if __name__=='__main__':
    ...
    for dataset_name in ["bbbp", "bace", "freesolv", "lipophilicity", "esol","toxcast", "clintox", "sider", "tox21", "muv"]:
        print(f"""Train on {dataset_name}""")
        config = configs[dataset_name]
        config['similarity_sub'] = 0.1
